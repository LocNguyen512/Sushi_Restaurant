...
for order in order_data:
    ...
    # Kiểm tra và phân tích ORDER_DATE với định dạng chỉ ngày
    try:
        order_date = datetime.strptime(order_date_str, '%Y-%m-%d')  # Sử dụng định dạng chỉ có ngày
        print(f"Đã phân tích ORDER_DATE: {order_date}")
    except ValueError:
    ...
    print(f"Chọn bàn TABLE_NUM: {table_number} cho đơn hàng ORDER_ID: {order_id}")

    # Kiểm tra nếu có đơn hàng Offline khác cùng branch_id và ORDER_TIME, phải khác bàn
    table_conflict = any(o['TABLE_NUMBER'] == table_number and o['BRANCH_ID'] == branch_id and o['ORDER_TIME'] == order_date for o in offline_order_data)
    if table_conflict:
        print(f"Xung đột bàn với đơn hàng ORDER_ID: {order_id}, tìm bàn khác...")
        while table_conflict:
            table_number = random.choice(available_tables)['TABLE_NUM']
            table_conflict = any(o['TABLE_NUMBER'] == table_number and o['BRANCH_ID'] == branch_id and o['ORDER_TIME'] == order_date for o in offline_order_data)
            print(f"Đã chọn lại bàn TABLE_NUM: {table_number} cho đơn hàng ORDER_ID: {order_id}")

    # Tìm nhân viên làm việc tại chi nhánh và thời gian hợp lệ
    available_employees = work_history_data.get(branch_id, [])
    valid_employees = [e['EMPLOYEE_ID'] for e in available_employees if e['START_DATE'] <= order_date <= e['END_DATE']]
    
    if valid_employees:
        employee_id = random.choice(valid_employees)
        print(f"Chọn nhân viên EMPLOYEE_ID: {employee_id} cho đơn hàng ORDER_ID: {order_id}")
    else:
        print(f"Không tìm thấy nhân viên phù hợp cho đơn hàng ORDER_ID: {order_id}")
    ...
    if random.random() <= 0.7:
        ...
        employee_rating = random.randint(1, 4)
    print(f"Chọn xếp hạng nhân viên EMPLOYEE_RATING: {employee_rating} cho đơn hàng ORDER_ID: {order_id}")
...
print("Đang ghi dữ liệu xuống file oforder_data.csv...")
header = ['OFORDER_ID', 'TABLE_NUMBER', 'EMPLOYEE_ID', 'BRANCH_ID', 'EMPLYEE_RATING', 'ORDER_TIME']
with open('oforder_data.csv', mode='w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=header)
    writer.writeheader()
    for row in offline_order_data:
        writer.writerow(row)

print("Dữ liệu đã được ghi xuống file oforder_data.csv.")
